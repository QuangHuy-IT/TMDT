...
import { formatFlashSaleDate } from '../../utils/flashSaleTime';

const FlashSaleDateSelector = ({ days, selectedDate, onDateClick, className }) => {
  ...
  const getCardStyle = (dayObj) => {
    const isActive = dayObj.dateStr === selectedDate;
    const isRunning = dayObj.isRunning;
    const isEnded = dayObj.isEnded;
    ...
    if (isActive && isEnded) {
      return 'bg-white border-slate-300 text-slate-500 shadow-sm ring-2 ring-slate-300/50 cursor-not-allowed opacity-50';
    }
    if (isRunning) {
      return 'bg-white/20 border-white/30 text-white hover:bg-white/30 hover:border-white/50 backdrop-blur-sm';
    }
    if (isEnded) {
      return 'bg-white/5 border-white/10 text-white/40 backdrop-blur-sm cursor-not-allowed opacity-50';
    }
    return 'bg-white/10 border-white/20 text-white/70 hover:bg-white/20 hover:border-white/35 backdrop-blur-sm';
  };

  const getStatusBadge = (dayObj) => {
    const isActive = dayObj.dateStr === selectedDate;
    const isRunning = dayObj.isRunning;
    const isEnded = dayObj.isEnded;
  ...
  return (
    <div className={cn("flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide", className)}>
      {days.map((dayObj) => {
        const { day, month, weekday } = formatDate(dayObj.repSession.startAt);
        const status = getStatusBadge(dayObj);
        const isActive = dayObj.dateStr === selectedDate;

        return (
          <button
            key={dayObj.dateStr}
            disabled={dayObj.isEnded}
            onClick={() => onDateClick(dayObj.dateStr)}
            className={cn(
          ...
          >
            ...
            {/* Status badge */}
            <span className={cn(
              "text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider w-full text-center mt-1",
              status.class
            )}>
              {dayObj.isRunning && !isActive && (
